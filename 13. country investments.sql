...
a.ID AS ID, 
DURATION,
cast(substring(phone_number, 1, 3) as int) as countrycode 
from Person A
inner join (
...
name,
avg(duration) over(partition by countrycode) as countryavg,
avg(duration) over() as totalavg
from cte A
join country B
on A.countrycode = B.country_code
)
select name from cte2 where countryavg > totalavg
